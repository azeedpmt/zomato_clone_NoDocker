...
const AddAddressPage = () => {
    ...
    // 🗑 Delete address - Using POST method (matching backend)
    const deleteAddress = async (id: string) => {
        if (!window.confirm("Delete this address?")) return;
        try {
            setDeletingId(id);
            // Changed from DELETE to POST to match backend route
            const response = await axios.post(
                `${restaurantService}/api/address/${id}`,
                {}, // Empty body since backend doesn't seem to need data
                {
                    headers: {
                        Authorization: `Bearer ${localStorage.getItem("token")}`,
                    },
