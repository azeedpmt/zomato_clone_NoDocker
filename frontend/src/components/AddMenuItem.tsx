...
const AddMenuItem = ({onItemAdded}:{onItemAdded:()=> void}) => {
  ...
  return (
    ...
    <button disabled={loading} onClick={handleSubmit} className="w-full rounded-lg text-white text-sm py-3 font-semibold transition bg-red-500 cursor-pointer">{loading ? "adding...":"Add Item"} </button>
    </div>
  );
};
