...
const AdminRestaurantCard = ({restaurant,onVerify}:{ restaurant:any; onVerify:()=>void;}) => {
  const verify =async()=>{
    try {
      ...
      toast.error("failed ot verify restaurent")
    }
  }
  return (
    <div className="rounded-xl bg-white p-4 shadow space-y-2">
      ...
      <button className='w-full rounded bg-green-500 py-2 text-white hover:bg-green-600' onClick={verify}>Verify Restaurant</button>
    </div>
  )
}
