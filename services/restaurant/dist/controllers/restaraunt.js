...
export const fetchMyRestauarant = TryCatch(async (req, res) => {
    if (!req.user) {
        return res.status(401).json({
...
export const updateRestaurant = TryCatch(async (req, res) => {
    ...
    const restaurant = await Restaurant.findOneAndUpdate({ ownerId: req.user._id }, { $set: { name, description } }, { new: true });
    if (!restaurant) {
