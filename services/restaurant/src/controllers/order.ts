...
export const createOrder=TryCatch(async(req:AuthentictedRequest,res)=>{
    const user=req.user;
    if(!user){
...
export const fecthRestaurantOrders=TryCatch(async(req:AuthentictedRequest,res)=>{
    const user=req.user;
...
export const getMyOrders=TryCatch(async(req:AuthentictedRequest,res)=>{
...
export const fetchSingleOrder=TryCatch(async(req:AuthentictedRequest,res)=>{
    if(!req.user){
        return res.status(401).json({
            message:"Unauthorized",
        });
