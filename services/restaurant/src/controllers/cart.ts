...
export const addToCart =TryCatch(async(req:AuthentictedRequest,res)=>{
    if(!req.user){
...
export const fetchMyCart=TryCatch(async(req:AuthentictedRequest,res)=>{
    if(!req.user){
        return res.status(401).json({
            message:"Please Login",
        });
...
export const incrementCartItem = TryCatch(async(req:AuthentictedRequest,res)=>{
    const userId=req.user?._id;
...
export const decrementCartItem = TryCatch(async(req:AuthentictedRequest,res)=>{
    const userId = req.user?._id;
    const { itemId } = req.body;
...
export const clearCart =TryCatch(async(req:AuthentictedRequest,res)=>{
    const userId=req.user?._id;
    if(!userId){
        return res.status(401).json({
...
export const removeCartItem = TryCatch(async(req:AuthentictedRequest,res)=>{
    const userId=req.user?._id;
    const {itemId}=req.body;
